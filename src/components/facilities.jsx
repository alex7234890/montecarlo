      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {fields.map((field, i) => (
            ...
            >
              <div className="glass p-6 sm:p-8 h-full border border-foreground/5">
                {/* Header row */}
                <div className="flex flex-col sm:flex-row items-start justify-between gap-4 mb-6">
                  <div>
                    <h3 className="font-heading text-3xl sm:text-4xl text-foreground">{field.name}</h3>
                    <p className="text-primary text-xs sm:text-sm font-semibold">{field.subtitle}</p>
                  </div>
                  <div className="glass rounded-lg px-3 sm:px-4 py-2 flex items-center gap-2 shrink-0">
                    <Maximize2 className="text-accent" size={14} />
                    <span className="text-foreground text-xs sm:text-sm font-semibold">{field.size}</span>
                  </div>
                </div>
